...
async function generateAICaption(product) {
    ...
    try {
        const prompt = `คุณคือ Content Creator มือโปร
จงเขียนแคปชั่นสำหรับสินค้าชิ้นนี้ โดยมีกฎเหล็กดังนี้:
1. **จัดรูปแบบบรรทัด**: "ต้อง" มีการเว้นบรรทัดระหว่างส่วนต่างๆ ให้ชัดเจน (ห้ามเขียนติดกันเป็นก้อน)
2. **ห้ามตอบอย่างอื่น**: ตอบกลับมา "เฉพาะคุณสมบัติและแคปชั่น" ตามรูปแบบด้านล่างเท่านั้น

ข้อมูลสินค้า:
- ชื่อสินค้า: "${title}"
- ราคา: ${price} บาท

รูปแบบคำตอบที่คุณต้องเป๊ะ (Copy & Paste ได้เลย):
✨ [ชื่อสินค้าสั้นๆ]

[ประโยคป้ายยา/รีวิวสั้นๆ]
